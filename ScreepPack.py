...
def validateConfigTypes(config):
    if "output_directory" not in config:
        raise ImportError("'output_directory' parameter not specified in screep-pack.json, please specify an output directory")
    if "screeps_folder" not in config:
        raise ImportError("'screeps_folder' parameter not specified in screep-pack.json, please specfify where the screeps root source folder is")
...
def flatten_screep_files():
    try:
        with open('screep-pack.json', 'r') as jsonConfig:
        ...
        print("screep-pack.json file not found. Ensure it exists in the same directory as ScreepPack.py")
